 ...
 */
void nitro::CubicInterpolatorySampler::computeMonotonicTangents(CbdImage &image, int x,
                                                         int y) {
  int n = image.numLevels();
  auto &distanceField = image.getDistField();
...
float nitro::CubicInterpolatorySampler::distFunc(CbdImage &image, int x, int y,
                                          float p, int numLevelsInput) const {
  auto &sdf = image.getDistField();
  numLevelsInput -= 1;
...
float nitro::CubicInterpolatorySampler::distFuncIndexed(CbdImage &image, int x, int y,
                                                 float p,
...
nitro::CbdImage nitro::CubicInterpolatorySampler::resample(CbdImage &image,
  ...
  for (int y = 0; y < height; y++) {
    for (int x = 0; x < width; x++) {
      ...
      computeMonotonicTangents(image, x, y);

      for (int d = numDesiredLevels - 1; d >= 0; d--) {
        float p = d / (numDesiredLevels - 1.0f);
        float dist;
        if (image.isIndexed()) {
          dist = distFuncIndexed(image, x, y, p, numLevelsInput);
        } else {
          dist = distFunc(image, x, y, p, numLevelsInput);
        }
        if (dist <= 0.0f) {

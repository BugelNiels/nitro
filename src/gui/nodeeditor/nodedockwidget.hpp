...
/**
 * A dock widget used for the node editor view.
 */
class NodeDockWidget : public QDockWidget {
public:
    /**
     * Creates a new node dock with the corresponding view from the provided node registry and adds it to the MainWindow.
    ...
    /**
     * Clears the graph model and removes all the nodes.
     */
    void clearModel();

    /**
     * Checks whether the user can safely quit from the application without losing unsaved changes.
     * @return True if the user can quit safely; false otherwise.
     */
    bool canQuitSafely();

    /**
     * Attempts to save the node graph. If the model has not been saved yet, it will always ask the user for a file location.
    ...
    /**
     * Retrieves the file name of the saved file.
     * @return The file name of the saved file.
     */
    const QString &getFileName();

    /**
     * Refreshes the entire node graph view.
     */

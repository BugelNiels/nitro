...
JpegOperator::JpegOperator(QLabel *valueLabel, QLabel *originalSizeLabel, QLabel *ratioLabel)
...
std::function<std::unique_ptr<NitroNode>()> JpegOperator::creator(const QString &category) {
    return [category]() {
        ...
        return builder
                .withOperator(
                        std::make_unique<JpegOperator>(valueLabel, originalSizeLabel, crLabel))
                ->withIcon("zip.png")

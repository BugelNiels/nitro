...
const vec3 backgroundCol = vec3(0);
const vec3 matCol = vec3(0.5, 0.8, 1);
const int numSteps = 256;

vec3 center = vec3(modelviewmatrix * vec4(vec3(0, 0, .1), 1));
...
float sdf(vec3 pos) {
  return mix(sdfSphere(pos, center),
             sdfSphere(pos, center + vec3(radius, 0, 0)), .5);
}
